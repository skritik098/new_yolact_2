...
from new_imageprocessor import ImageProcessor
...
def evaLanevideo(net, path:str, out_path:str=None):
    ...
    while(vid.isOpened()):
        ...
        dst_points = np.float32([[xl_bottom,frame.shape[0]], [xl_bottom, 0], [xr_bottom, 0], [xr_bottom, frame.shape[0]]])
        lane_detector = ImageProcessor(img_points, obj_points, mtx, dist, src_points, dst_points)
        frame = cv2.cvtColor(lane_detector.lane_detection(frame), code=cv2.COLOR_BGR2RGB)
